...
public class Server 
{
    public static void main(String... args)
    {
        try
        {
            ...
            server.createContext("/assets",new FileHandler(root, false));
            server.createContext("/",new FileHandler(root, true));
            server.setExecutor(null);
            System.out.println("Server running on port " + port + " with root " + root);
            server.start();
        }
    ...
    public void handle(HttpExchange exchange)
    {
        try 
        {
            if(exchange.getRequestMethod().equals("GET"))
            {
                ...
                Connection conn = DriverManager.getConnection ("jdbc:h2:./test"); 
                PreparedStatement preSt = conn.prepareStatement("select * from People where userId = ?");
...
class FileHandler implements HttpHandler {
    public FileHandler(String root, boolean onlyIndex){
        this.root = root;
        this.onlyIndex = onlyIndex;
    }

    private String root;
    private boolean onlyIndex = false;
    ...
    public void handle(HttpExchange t) throws IOException {
        try
        {
            ...
            if(uri.getPath().equals("/") || onlyIndex)
            {
            ...
            Class me = Class.forName("FileHandler");

            InputStream stream = me.getResourceAsStream(root + uri);
            ...
            if (stream == null){
                ...
                t.sendResponseHeaders(200, getInputLength(stream));

                OutputStream os = t.getResponseBody();
                stream = me.getResourceAsStream(root + uri);
    ...
    static long getInputLength(InputStream stream) throws IOException {
